...
def _build_native_code_prompt(context: dict[str, Any]) -> str:
    return (
        ...
        "batch_loss_trace when they exist. Important: functions created by injected code may run "
        "later with the frame globals, not the transient exec locals. Do not create helper lambdas "
        "or functions that reference transient exec locals such as l = locals(). If installing callbacks, "
        "hooks, wrappers, or helpers, bind state through default arguments or store it on a long-lived "
        "runtime object/global before the function is called later. Do not import unavailable "
        "packages. Do not ask questions. Include a short print(..., flush=True) so the operator "
        "can see what changed or observed immediately.\n\n"
